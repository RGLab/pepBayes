...
int Hull<T>::drawSample(RngStream rng, double & x_sample) {
    ...
    while (num_trials < MAX_HULL_TRIALS) {
        ...
        if (!R_FINITE(x_trial) || x_trial <= 0.0) {
            printHull();
            Rcpp::Rcout << "Density parameters:" << std::endl;
            dist.printParameters();
            Rcpp::Rcout << "non-positive trial value in Hull<T>::drawSample:" <<
                    std::endl;
            Rcpp::Rcout << "x_trial = " << x_trial << std::endl;
            Rcpp::stop("");
        }
        // apply squeeze test
...
*/
    ...
    if (hp_x > 0.0) {
        x_star = logspaceAdd(
                log(p_remainder) + hull_integral + log(hp_x),
                    (z_prev - x) * hp_x + h_x - upper_hull_max) +
                            x*hp_x - h_x + upper_hull_max;
    } else {
        x_star = logspaceSubtract(
                (z_prev - x) * hp_x + h_x - upper_hull_max,
                log(p_remainder) + hull_integral + log(abs(hp_x))) +
                        x*hp_x - h_x + upper_hull_max;
    }
...
void Hull<T>::printHull(){
    Rcpp::Rcout << fixed;
    Rcpp::Rcout << setprecision(4);
    Rcpp::Rcout << "hull max value:" << upper_hull_max << endl;
    Rcpp::Rcout << setw(7) << "seg idx" << setw(14) << "x" << setw(14) << "z";
    Rcpp::Rcout << setw(14) << "seg prob" << setw(14) <<"cumu prob";
    Rcpp::Rcout << setw(14) << "seg int" << setw(14) << "cumu int" << endl;
    for (int i = 0; i < num_hull_segments; i++) {
        Rcpp::Rcout << setw(7) << i;
        Rcpp::Rcout << setw(14) << hull[i].left_x;
        Rcpp::Rcout << setw(14) << hull[i].z;
        Rcpp::Rcout << setw(14) << exp(hull[i].prob);
        Rcpp::Rcout << setw(14) << exp(hull[i].cum_prob);
        Rcpp::Rcout << setw(14) << exp(hull[i].raw_integral);
        Rcpp::Rcout << setw(14) << exp(hull[i].raw_cumulative_integral) << endl;
    }
}
#endif /* HULL_H_ */
